...
ig.module('game.menus.title').requires('game.menus.base', 'game.menus.detailed-stats', 'game.ease').defines(function() {
	...
	MenuTitle = Menu.extend({
		...
		init: function() {
			...
			if (window.Cocoon) {} else if (ig.ua.iOS || ig.ua.android) {
		...
		draw: function() {
			...
			if (this.items.length > 1) {
				this.items[1].y = ig.ease.inOutQuad(this.transition, ig.system.height / this.scale - 160, 1900, 1);
			}
			if (this.items.length > 2) {
				this.items[2].y = ig.ease.inOutQuad(this.transition, ig.system.height / this.scale - 100, 2100, 1);
			}
...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function() {
	EntityEnemy = ig.Entity.extend({
		...
		init: function(x, y, settings) {
			...
			this.word = this.getRandomWord(); //settings.word || this.getWordWithLength(length);
			...
			if (!ig.ua.mobile) {
				this.speed *= 2 * ig.game.speedFactor;
			}
		},
		getRandomWord: function() {
		    var m = ig.WORDS.PROG['c'];
...
ig.module('game.words.en').defines(function() {
	...
	ig.WORDS.PROG = {
	    'c': ["auto", "break", "case", "char", "const", "continue", "default", "do", "double", "else", "enum", "extern", "float", "for", "goto", "if", "int", "long", "register", "return", "short", "signed", "sizeof", "static", "struct", "switch", "typedef", "union", "unsigned", "void", "volatile", "while", "if", "for", "do", "while", "int", "float", "return", "if", "for", "do", "while", "int", "float", "return"]
	};
});
	...
	ZType = ig.Game.extend({
		...
		keydown: function( ev ) {
		    if ( ev.altKey && ev.keyCode == 13 ) {
		        var i = document.getElementById( "ztype-game-canvas" );
			...
			if ( c === ig.KEY.LEFT_ARROW ) {
			    this.player.onLeftDown();
			    return false;
			}
			if ( c === ig.KEY.RIGHT_ARROW ) {
			    this.player.onRightDown();
			    return false;
			}
			if ( c === ig.KEY.UP_ARROW ) {
			    this.player.onUpDown();
			    return false;
			}
			if ( c === ig.KEY.DOWN_ARROW ) {
			    this.player.onDownDown();
			    return false;
			}
